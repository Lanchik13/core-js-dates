...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const time = Date.parse(dateEnd) - Date.parse(dateStart);
...
function isDateInPeriod(date, period) {
  ...
  return (
    Date.parse(period.end) >= currentDate &&
    Date.parse(period.start) <= currentDate
  );
}
...
function getWeekNumberByDate(date) {
  ...
  const daysBetween = (date - startOfYear) / (24 * 60 * 60 * 1000);
  const startDayOfYear = startOfYear.getDay() || 7;

  return Math.ceil((daysBetween + startDayOfYear) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  return getNextFridayThe13th(
    new Date(currentDate.getFullYear() + 1, currentDate.getMonth(), 13)
  );
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const schedule = [];
  const patternLength = countWorkDays + countOffDays;
  ...
  for (
    ...
    if (i % patternLength < countWorkDays) {
      schedule.push(newDate(d));
    }
  }
